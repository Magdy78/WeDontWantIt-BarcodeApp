...
// Function to handle the country lookup
const lookupCountry = () => {
    const barcodeInput = document.getElementById('barcodeInput').value;
    const resultElement = document.getElementById('result');

    if (barcodeInput) {
        const countryCodes = barcode_country(barcodeInput);
        if (countryCodes) {
            // Check if the country is in the specified list
            const prohibitedCountries = ['US', 'IL', 'GB', 'FR', 'DE', /* Add more countries as needed */];
            if (prohibitedCountries.includes(countryCodes.split(',')[0])) {
                resultElement.textContent = 'Not with us';
                // You can also show a popup or perform other actions here
            } else {
                resultElement.textContent = `Country Codes: ${countryCodes}`;
            }
        } else {
            resultElement.textContent = 'Country not found for the given barcode.';
        }
    } else {
        resultElement.textContent = 'Please enter a valid barcode.';
    }
};
